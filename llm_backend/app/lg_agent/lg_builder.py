...
async def analyze_and_route_query(
    ...
    这个函数使用语言模型对用户的查询进行分类，并根据分类结果确定路由。

    Args:
        state (AgentState): 当前代理状态，包括对话历史记录。
        config (RunnableConfig): 包含用于查询分析的模型配置。
...
async def check_hallucinations(
    ...
    """分析用户的查询并检查响应是否得到基于检索到的文档的事实集的支持；提供二进制分数结果。

    该函数使用语言模型分析用户的查询，并给出二进制评分结果。
    
    Args:
        state (AgentState): 当前代理状态，包括对话历史。
        config (RunnableConfig): 配置用于查询分析的模型。

    Returns:
        dict[str, GradeHallucinations]: 包含'hallucination'键的字典，其中包含二进制评分结果。
    """
    if settings.AGENT_SERVICE == ServiceType.DEEPSEEK:
        # 对于Agent服务，使用deepseek-chat而不是deepseek-reasoner
        agent_model = "deepseek-chat"
